...
export default function CourseDetailsPage({ params }) {
  ...
  const { currentCourse, courseSection, loading, error, courseVideoInfo, isEnrolled } = useSelector(
    (state) => state.course
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <div className="flex items-center space-x-4">
                ...
                {/* ✅ Updated: Use isEnrolled to show enrollment status */}
                {userRole === 'customer' ? (
                  isEnrolled ? (
                    <div className="bg-green-500 text-white px-8 py-4 rounded-xl font-bold text-lg border-2 border-green-400 flex items-center space-x-2">
                    ...
                    <button 
                      onClick={() => router.push(`/customer/course/${courseId}/payment`)}
                      className="bg-white text-gray-700 px-8 py-4 rounded-xl font-bold text-lg hover:bg-green-50 hover:text-green-800 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl border-2 border-white/20"
                    >
                      Enroll Now
                    </button>
                  )
                ) : userRole === 'seller' ? (
                  <div className="bg-white/20 text-white px-8 py-4 rounded-xl font-bold text-lg border-2 border-white/20 flex items-center space-x-2">
                  ...
                  <button className="bg-gray-500 text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-gray-600 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl border-2 border-gray-400">
                    Login to Enroll
                  </button>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center space-x-2 mb-1">
                                      {/* ✅ Updated: Only allow video access if enrolled or if it's a preview video */}
                                      {(isEnrolled && userRole === 'customer') || video.is_preview ? (
                                        ...
                                        <span className="text-sm font-semibold text-gray-500 truncate cursor-not-allowed">
                                          {video.title || `Video ${videoIndex + 1}`}
                                          {!isEnrolled && userRole === 'customer' && (
                                  ...
                                  {/* Action buttons - ✅ Updated: Show appropriate buttons based on enrollment */}
                    ...
                    currentCourse.requirements.map((requirement, index) => (
                      <div key={index} className="flex items-start space-x-3">
                        ...
                        >
                          <path
                            fillRule="evenodd"
                            d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.914L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                            clipRule="evenodd"
                          />
                        </svg>
            ...
            <div className="space-y-6 ">
              ...
              <div className="bg-gray-800 rounded-2xl shadow-xl border-2 border-red-100 p-8 sticky top-6 hover:shadow-2xl transition-all duration-300">
                ...
                {/* ✅ Conditional sidebar buttons based on user role and enrollment status */}
                {userRole === 'customer' ? (
                  
                  isEnrolled ? (
                    // ✅ Show enrolled status
                    <>
                      <div className="w-full bg-green-500 text-white py-4 rounded-xl font-bold text-lg mb-4 text-center border-2 border-green-400 flex items-center justify-center space-x-2">
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                        <span>Enrolled</span>
                      ...
                      <button className="w-full border-2 bg-blue-50 border-blue-300 text-blue-700 py-3 rounded-xl font-semibold hover:bg-blue-100 hover:border-blue-400 transition-all duration-300 mb-6">
                        Continue Learning
                      </button>
                    </>
                  ) : (
                    // ✅ Show enrollment buttons if not enrolled
                    <>
                      <button 
                        onClick={() => router.push(`/customer/course/${courseId}/payment`)}
                        className="w-full bg-gradient-to-r from-white to-gray-50 text-black py-4 rounded-xl font-bold text-lg hover:from-green-400 hover:to-green-500 hover:text-white transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl mb-4"
                      >
                        Enroll Now
                      </button>
                  ...
                  <>
                    <div className="w-full bg-gray-700 text-gray-300 py-4 rounded-xl font-bold text-lg mb-4 text-center border-2 border-gray-600 flex items-center justify-center space-x-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                      </svg>
                    ...
                    <div className="w-full bg-gray-700 text-gray-300 py-3 rounded-xl font-semibold mb-6 text-center border-2 border-gray-600 flex items-center justify-center space-x-2">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
                      </svg>
                      <span>Instructors cannot enroll</span>
                    </div>
                  </>
                ) : (
                  <>
                    <button className="w-full bg-gray-500 text-white py-4 rounded-xl font-bold text-lg hover:bg-gray-600 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl mb-4">
                      Login to Enroll
                    </button>

                    <button className="w-full border-2 bg-gray-100 border-gray-300 text-gray-600 py-3 rounded-xl font-semibold hover:bg-gray-200 transition-all duration-300 mb-6">
